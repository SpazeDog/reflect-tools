...
EOF
        ...
        cd "$BUILD_PATH"
        zip "$BUILD_BINTRAY/sources.jar" "xposed-api.jar" || exit 1

        cp "$BUILD_PATH/projects/reflectTools-$BUILD_TYPE.aar" "$BUILD_BINTRAY/library.aar" || exit 1

        btPackage="reflect-tools"
        btGroupId="com.spazedog.lib"
        btVersion="$(grep 'versionName' "$BUILD_PATH/projects/reflectTools/build.gradle" | sed 's/.*\"\(.*\)\"/\1/')"
